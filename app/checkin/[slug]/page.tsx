...
export default function CheckinPage() {
  ...
  const handleAddOccupant = () => {
    setOccupants([
      ...occupants,
      { name: '', birth_date: '', relation: 'Istri', phone: '', address: '', ktpFile: null }
    ]);
  };
  ...
  const handleOccupantChange = (index: number, field: keyof OccupantInput, val: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // KTP untuk anggota (jika ada dan umur >= 17)
    if (hasMembers) {
      occupants.forEach((occ, idx) => {
        const memberIndex = idx + 1;
        if (occ.ktpFile) {
          formData.append(`member_ktp_${memberIndex}`, occ.ktpFile);
        }
      });
    }
  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        {/* HEADER PROPERTI DENGAN TOMBOL ZOOM MULTI-STEP */}
        <div className="bg-emerald-800 text-white p-5 md:p-7 rounded-3xl shadow-xl space-y-3">
          ...
          <div>
            <h1 className="text-[1.4rem] font-black text-white">{property.name || property.property_name}</h1>
            <p className="text-[0.8rem] text-emerald-100 mt-1 font-medium">{property.address || 'Lingkungan RT Setempat'}</p>
          </div>

          {managerContact && (
            <div className="pt-2 border-t border-emerald-700/80 flex items-center justify-between">
              <span className="text-[0.8rem] text-emerald-100 font-medium">Pengelola: <b>{managerName}</b></span>
              <a
                href={`https://wa.me/${managerContact.replace(/\D/g, '')}`}
                target="_blank"
                rel="noopener noreferrer"
                className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-[0.75rem] inline-flex items-center gap-1 shadow"
              >
                💬 Chat WA Pengelola
              </a>
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-white p-5 md:p-6 rounded-3xl shadow-md border-2 border-slate-200 space-y-4">
            <div className="border-b-2 border-slate-100 pb-2 flex justify-between items-center">
              <h3 className="font-black text-slate-900 text-[0.95rem] uppercase flex items-center gap-2">
                <span>1️⃣</span> Lokasi Unit & Kontak Utama
              </h3>
              <span className="text-[0.7rem] font-black px-2.5 py-0.5 bg-red-100 text-red-700 rounded-full">WAJIB SEMUA</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[0.85rem] font-bold text-slate-800 mb-1">Nomor WhatsApp Aktif *</label>
                <input
                  type="tel"
                  required
              ...
              <div>
                <label className="block text-[0.85rem] font-bold text-slate-800 mb-1">Nomor Kamar / Unit Hunian *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full p-3 border-2 border-slate-200 rounded-2xl outline-none text-[0.95rem] focus:border-emerald-600 bg-white font-bold text-emerald-900"
                />
              </div>

              <div>
                <label className="block text-[0.85rem] font-bold text-slate-800 mb-1">Tanggal Mulai Menetap *</label>
                <input
                  type="date"
                  required
                  value={entryDate}
                  onChange={(e) => setEntryDate(e.target.value)}
                  className="w-full p-3 border-2 border-slate-200 rounded-2xl outline-none text-[0.95rem] bg-white font-medium"
                />
              </div>

              <div>
                <label className="block text-[0.85rem] font-bold text-slate-800 mb-1">Alamat Asal Sesuai KTP *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full p-3 border-2 border-slate-200 rounded-2xl outline-none text-[0.95rem] focus:border-emerald-600 bg-white"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-5 md:p-6 rounded-3xl shadow-md border-2 border-slate-200 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[0.85rem] font-bold text-slate-800 mb-1">Nama Lengkap (Sesuai KTP) *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full p-3 border-2 border-slate-200 rounded-2xl outline-none text-[0.95rem] focus:border-emerald-600 bg-white font-bold"
                />
              </div>

              <div>
                <label className="block text-[0.85rem] font-bold text-slate-800 mb-1">Tanggal Lahir *</label>
                <input
                  type="date"
                  required
                  value={birthDate}
                  onChange={(e) => setBirthDate(e.target.value)}
                  className="w-full p-3 border-2 border-slate-200 rounded-2xl outline-none text-[0.95rem] bg-white"
                />
              </div>

              <div>
                <label className="block text-[0.85rem] font-bold text-slate-800 mb-1">Pekerjaan *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-[0.85rem] font-bold text-slate-800 mb-1">Status Pernikahan *</label>
                <select
                  value={maritalStatus}
                  onChange={(e) => setMaritalStatus(e.target.value)}
                  className="w-full p-3 border-2 border-slate-200 rounded-2xl outline-none text-[0.95rem] bg-white font-bold text-slate-900"
                >
              ...
              <div className="md:col-span-2">
                <label className="block text-[0.85rem] font-bold text-slate-800 mb-1">Foto KTP Penanggung Jawab (Usia ≥ 17 Tahun) *</label>
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => setKtpFile(e.target.files ? e.target.files[0] : null)}
                  className="w-full p-3 border-2 border-slate-200 rounded-2xl bg-slate-50 text-[0.8rem] font-semibold file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-[0.75rem] file:font-bold file:bg-slate-900 file:text-white"
                />
              </div>

              {maritalStatus === 'Menikah' && (
                <div className="md:col-span-2 bg-amber-50 p-4 rounded-2xl border-2 border-amber-300 space-y-2">
                  <label className="block font-black text-amber-950 text-[0.85rem]">
                  ...
                  <input
                    ...
                    className="w-full p-2.5 border-2 border-amber-200 rounded-xl bg-white text-[0.8rem] file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-[0.75rem] file:font-bold file:bg-amber-900 file:text-white"
                  />
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    />
                    <label htmlFor="docPending" className="text-[0.8rem] font-bold text-amber-900 cursor-pointer">
          ...
          {/* SEKSI 3: ANGGOTA TAMBAHAN DENGAN LABEL EKSPLISIT & LOGIKA KTP USIA >= 17 */}
          <div className="bg-white p-5 md:p-6 rounded-3xl shadow-md border-2 border-slate-200 space-y-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-[0.8rem] text-slate-500 font-medium">Centang jika menempati kamar bersama orang lain/keluarga.</p>
              </div>
              <input
                type="checkbox"
                checked={hasMembers}
            ...
            {hasMembers && (
              <div className="space-y-4 pt-3 border-t-2 border-slate-100">
                {occupants.map((occ, idx) => {
                  ...
                  return (
                    <div key={idx} className="p-4 md:p-5 bg-slate-50 rounded-2xl border-2 border-slate-200 space-y-3 relative">
                      <div className="flex justify-between items-center border-b pb-2">
                        <span className="text-[0.85rem] font-black text-slate-800">
                          Anggota #{idx + 1} {occ.birth_date && <span className="text-emerald-700 font-bold">({memberAge} Tahun)</span>}
                        </span>
                        <button
                          type="button"
                          onClick={() => handleRemoveOccupant(idx)}
                          className="text-[0.75rem] text-red-600 font-bold hover:underline"
                        >
                          ✕ Hapus
                        </button>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          <label className="block text-[0.8rem] font-bold text-slate-700 mb-1">Nama Lengkap Anggota *</label>
                          <input
                            type="text"
                            required
                            placeholder="Contoh: Almahyra"
                            value={occ.name}
                            onChange={(e) => handleOccupantChange(idx, 'name', e.target.value)}
                            className="w-full p-2.5 border-2 border-slate-200 rounded-xl bg-white text-[0.85rem] font-bold"
                          />
                        </div>

                        <div>
                          <label className="block text-[0.8rem] font-bold text-slate-700 mb-1">Tanggal Lahir *</label>
                          <input
                            type="date"
                            required
                            value={occ.birth_date}
                            onChange={(e) => handleOccupantChange(idx, 'birth_date', e.target.value)}
                            className="w-full p-2.5 border-2 border-slate-200 rounded-xl bg-white text-[0.85rem]"
                          />
                        </div>

                        <div>
                          <label className="block text-[0.8rem] font-bold text-slate-700 mb-1">Hubungan / Status *</label>
                          <select
                            value={occ.relation}
                            onChange={(e) => handleOccupantChange(idx, 'relation', e.target.value)}
                            className="w-full p-2.5 border-2 border-slate-200 rounded-xl bg-white text-[0.85rem] font-bold text-slate-800"
                          >
                            <option value="Istri">Istri</option>
                            <option value="Suami">Suami</option>
                            <option value="Anak">Anak</option>
                            <option value="Saudara">Saudara / Keluarga</option>
                            <option value="Rekan / Teman">Rekan / Teman</option>
                          </select>
                        </div>

                        <div>
                          <label className="block text-[0.8rem] font-bold text-slate-700 mb-1">Nomor WhatsApp (Opsional)</label>
                          <input
                            type="tel"
                            placeholder="08123456789"
                            value={occ.phone}
                            onChange={(e) => handleOccupantChange(idx, 'phone', e.target.value)}
                            className="w-full p-2.5 border-2 border-slate-200 rounded-xl bg-white text-[0.85rem] font-mono"
                          />
                        </div>

                        {/* KONDISIONAL UPLOAD KTP ANGGOTA JIKA USIA >= 17 */}
                        {isAdult && (
                          <div className="md:col-span-2 bg-emerald-50 p-3.5 rounded-xl border-2 border-emerald-200 space-y-1.5">
                            ...
                            <input
                              ...
                              className="w-full p-2 border-2 border-emerald-300 rounded-xl bg-white text-[0.75rem] file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-[0.75rem] file:font-bold file:bg-emerald-900 file:text-white"
                            />
                          </div>
                        )}
                      </div>
                ...
                <button
                  ...
                  className="w-full py-3 bg-slate-200 hover:bg-slate-300 text-slate-900 font-black text-[0.85rem] rounded-2xl transition-all cursor-pointer"
                >
          ...
          <div className="bg-white p-5 md:p-6 rounded-3xl shadow-md border-2 border-slate-200 space-y-4">
            <div className="border-b-2 border-slate-100 pb-2 flex justify-between items-center">
              ...
              <span className="text-[0.7rem] font-black px-2.5 py-0.5 bg-red-100 text-red-700 rounded-full">WAJIB CENTANG</span>
            </div>

            <div className="p-4 bg-slate-50 border-2 border-slate-200 rounded-2xl max-h-40 overflow-y-auto font-mono text-[0.8rem] text-slate-800 leading-relaxed font-semibold">
            ...
            <div className="space-y-3 pt-2 text-slate-900">
              <div className="flex items-start gap-2.5">
                ...
                />
                <label htmlFor="agree1" className="font-bold text-[0.85rem] cursor-pointer leading-snug">
              ...
              <div className="flex items-start gap-2.5">
                <input
                  ...
                  className="w-5 h-5 mt-0.5 rounded text-emerald-600 cursor-pointer"
                />
                <label htmlFor="agree2" className="text-[0.8rem] text-slate-700 cursor-pointer leading-snug font-medium">
          ...
          <button
            ...
            className="w-full py-4 bg-emerald-700 hover:bg-emerald-800 text-white font-black text-[1rem] rounded-2xl transition-all shadow-lg disabled:bg-slate-300 cursor-pointer"
          >
