...
function isPhoneMatch(phoneA?: string, phoneB?: string): boolean { const a = cleanDigits(phoneA); const b = cleanDigits(phoneB); if (!a || !b) return false; if (a === b) return true; return (a.length >= 8 ? a.slice(-9) : a) === (b.length >= 8 ? b.slice(-9) : b); }
...
// LOGIKA RIWAYAT SEWA YANG SINKRON 100%
...
export default function OwnerDashboard() {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    } else { setLoginError(res.error || 'Nomor WhatsApp atau PIN 4-Digit salah.'); }
  };
  ...
  const handlePropFormSubmit = async (e: React.FormEvent) => {
    ...
    if (editingProperty) {
      ...
      if (res.success) {
        alert(`✅ Data properti "${propName}" berhasil diperbarui.`); setEditingProperty(null); setShowAddPropModal(false);
      ...
      if (!propPin || !/^\d{4}$/.test(propPin)) { alert('PIN harus berupa 4-digit angka'); setSubmittingProp(false); return; }
      ...
      if (res && res.success && res.data) {
        alert(`✅ Properti "${propName}" berhasil didaftarkan.`); setShowAddPropModal(false);
      ...
      } else { alert('Gagal membuat properti: ' + (res?.error || 'Kesalahan teknis')); }
    }
  };
  ...
  const handleAddExpenseSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success && res.data) {
      ...
      alert(`✅ Pengeluaran "${expenseTitle}" sebesar Rp ${parsedAmount.toLocaleString('id-ID')} tersimpan.`);
    } else { alert('Gagal mencatat pengeluaran: ' + res.error); }
  };
  ...
  const handleSaveTenantSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      setEditingTenant(null); alert(`✅ Data penyewa ${tenantName} diperbarui.`);
    } else { alert('Gagal update data penyewa: ' + res.error); }
  };
  ...
  const handleShareWA = (prop: Property) => {
    ...
    const message = `Halo calon penghuni *${prop.name || prop.property_name}*,\n\nSesuai Peraturan Wajib Lapor RT setempat, mohon melengkapi formulir lapor diri resmi melalui tautan berikut:\n\n👉 ${checkinUrl}\n\nTerima kasih.`;
  ...
  const handleSaveRules = async () => {
    ...
    if (res && res.success) {
      alert('✅ Tata tertib hunian berhasil diperbarui.'); setEditingRulesProp(null);
      if (activeProperty) setActiveProperty({ ...activeProperty, house_rules: rulesText });
    } else { alert('Gagal menyimpan tata tertib: ' + (res?.error || 'Kesalahan database')); }
  };
            ...
            {loadingDetails ? (
              ...
              <div className="space-y-5">
                ...
                <div className="flex border-b-2 border-slate-200 gap-2">
                  ...
                  <button onClick={() => setActiveTab('pengeluaran')} className={`py-3 px-5 text-[0.85rem] font-black rounded-t-2xl border-t-2 border-l-2 border-r-2 ${activeTab === 'pengeluaran' ? 'bg-white border-slate-300 text-red-800 -mb-0.5' : 'bg-slate-100 border-transparent text-slate-600'}`}>📈 Pengeluaran</button>
                      ...
                      <table className="w-full text-left border-collapse text-[0.8rem]">
                        <thead>
                          <tr className="bg-slate-100 border-b-2 text-slate-700 font-black uppercase text-[0.7rem]">
                            ...
                            <th className="p-3">Kamar & Masuk</th>
                            <th className="p-3">Status Tagihan</th>
                            <th className="p-3">Riwayat (3 Bulan)</th>
                          ...
                          {tenants.map((t) => {
                            ...
                            return (
                              <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                                <td className="p-3"><div className="font-black text-slate-900">{t.name}</div><span className="text-[0.65rem] font-bold px-2 bg-slate-100 rounded-md border">{t.relation || 'Anggota'}</span></td>
                                <td className="p-3"><span className="font-bold text-emerald-900 block">{t.room_number || '-'}</span><span className="text-[0.65rem] text-slate-500 font-mono">Masuk: {t.entry_date}</span></td>
                                <td className="p-3">{isHeadPerson ? (<button onClick={() => handleTogglePaymentStatus(t.id, t.payment_status)} className={`px-2.5 py-1 rounded-xl text-[0.7rem] font-black ${isPaid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{isPaid ? '✅ LUNAS' : '❌ BELUM'}</button>) : (<span className="text-slate-400 font-bold italic">- (Ikut PJ)</span>)}</td>
                ...
                {/* ... BAGIAN TAB PENGELUARAN & MATRIX SAMA SEPERTI SEBELUMNYA ... */}
              </div>
            ) : null}
          </div>
        )}
