...
export default function OwnerDashboard() {
  ...
  const handlePropFormSubmit = async (e: React.FormEvent) => {
    ...
    if (editingProperty) {
      ...
      setShowAddPropModal(false); alert('Dibuat.'); window.location.reload();
    }
  };
  ...
  if(!isLoggedIn) return (
    <main className="min-h-screen bg-slate-50 p-4 font-sans flex items-center justify-center">
      <form onSubmit={handleLoginSubmit} className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 max-w-sm w-full space-y-5">
        ...
        <button type="submit" disabled={loginLoading} className="w-full py-4 bg-slate-900 text-white font-black rounded-xl cursor-pointer hover:bg-slate-800">{loginLoading?'Memproses...':'Buka Dasbor'}</button>
      </form>
    </main>
  );
      ...
      {showAddPropModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-5 bg-slate-900 text-white flex justify-between"><h3 className="font-black">{editingProperty ? 'Edit Properti' : 'Tambah Properti'}</h3><button onClick={()=>{setShowAddPropModal(false);setEditingProperty(null);}} className="font-bold cursor-pointer">✕</button></div>
            <form onSubmit={handlePropFormSubmit} className="p-5 space-y-4 text-sm">
              <input type="text" required placeholder="Nama Kos" value={propName} onChange={e=>setPropName(e.target.value)} className="w-full p-3 border rounded-xl font-bold bg-slate-50" />
              ...
              <div className="p-4 border rounded-xl space-y-2"><p className="text-xs font-bold text-slate-500">Akses Nomor WA:</p><input type="tel" placeholder="WA Owner" value={propOwnerPhone} onChange={e=>setPropOwnerPhone(e.target.value)} className="w-full p-2 border rounded font-mono" /><input type="tel" placeholder="WA Pengelola" value={propManagerPhone} onChange={e=>setPropManagerPhone(e.target.value)} className="w-full p-2 border rounded font-mono" /></div>
              <input type="text" maxLength={4} required placeholder="PIN 4 Digit Akses" value={propPin} onChange={e=>setPropPin(e.target.value.replace(/\D/g,''))} className="w-full p-3 border border-slate-400 rounded-xl font-mono text-center tracking-widest font-black text-lg" />
              <button type="submit" disabled={submittingProp} className="w-full py-3.5 bg-slate-900 text-white font-black rounded-xl cursor-pointer">Simpan Data</button>
            </form>
          </div>
        </div>
      )}
