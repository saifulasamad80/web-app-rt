...
// RUMUS TERPUSAT: RIWAYAT 3 BULAN BERDASARKAN TANGGAL MASUK (SINKRON 100%)
...
export default function OwnerDashboard() {
  ...
  const handlePropFormSubmit = async (e: React.FormEvent) => {
    ...
    if (editingProperty) {
      ...
      if (res && res.success && res.data) {
        alert(`✅ Berhasil! Properti "${propName}" berhasil didaftarkan.`);
        setShowAddPropModal(false);
  ...
  const handleAddExpenseSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success && res.data) {
      ...
      setExpenseTitle('');
      setExpenseAmount('');
      alert(`✅ Berhasil Tersimpan!\n\nPengeluaran "${expenseTitle}" (${expenseCategory}) tercatat.`);
    } else {
  ...
  const handleSaveTenantSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      alert(`✅ Berhasil! Data penyewa ${tenantName} diperbarui.`);
    } else {
          ...
          <div className="max-w-md mx-auto space-y-4">
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-md border-2 border-slate-200 space-y-4 text-center">
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4 text-left">
                <input type="tel" required placeholder="08xxxxxxxxxx" value={loginPhone} onChange={(e) => setLoginPhone(e.target.value.replace(/\D/g, ''))} className="w-full p-3.5 border-2 border-slate-200 rounded-2xl outline-none font-mono text-[1rem] font-bold bg-white" />
                <input type="password" maxLength={4} required placeholder="• • • •" value={loginPin} onChange={(e) => setLoginPin(e.target.value.replace(/\D/g, ''))} className="w-full p-3.5 border-2 border-slate-200 rounded-2xl outline-none font-mono text-[1.2rem] font-bold tracking-[0.4em] text-center bg-white" />
                <button type="submit" disabled={loginLoading || !loginPhone || loginPin.length !== 4} className="w-full py-4 bg-emerald-700 hover:bg-emerald-800 text-white font-black text-[0.95rem] rounded-2xl cursor-pointer">Buka Dasbor Saya →</button>
              </form>
            </div>
          </div>
        ) : (
            ...
            <div className="bg-white p-4 md:p-5 rounded-3xl shadow-sm border-2 border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <select value={activeProperty?.id || ''} onChange={(e) => { const s = myProperties.find((p) => p.id === e.target.value); if(s) handleSelectProperty(s); }} className="p-2.5 border-2 border-emerald-400 rounded-2xl font-black text-emerald-950 bg-emerald-50 text-[0.9rem] outline-none cursor-pointer">
                  {myProperties.map((p) => (<option key={p.id} value={p.id}>🏠 {p.name || p.property_name} ({p.type})</option>))}
                </select>
              </div>
              <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-emerald-800 text-white p-6 rounded-3xl shadow-lg space-y-1">
                    <span className="text-[0.7rem] font-black uppercase tracking-wider text-emerald-200 bg-emerald-950/60 px-2.5 py-0.5 rounded-full">SEWA MASUK (LUNAS)</span>
                    <h3 className="text-[1.6rem] font-black text-white mt-1">Rp {totalRentCollected.toLocaleString('id-ID')}</h3>
                  </div>
                  <div className="bg-red-800 text-white p-6 rounded-3xl shadow-lg space-y-1">
                    <span className="text-[0.7rem] font-black uppercase tracking-wider text-red-200 bg-red-950/60 px-2.5 py-0.5 rounded-full">PENGELUARAN BULAN INI</span>
                    <h3 className="text-[1.6rem] font-black text-white mt-1">Rp {totalExpenses.toLocaleString('id-ID')}</h3>
                  </div>
                  {isOwner ? (
                    <div className="bg-amber-400 text-slate-950 p-6 rounded-3xl shadow-lg space-y-1 border-2 border-amber-500">
                      <span className="text-[0.7rem] font-black uppercase tracking-wider text-slate-900 bg-amber-200/80 px-2.5 py-0.5 rounded-full">LABA BERSIH BULAN INI</span>
                      <h3 className="text-[1.6rem] font-black text-slate-950 mt-1">Rp {netProfit.toLocaleString('id-ID')}</h3>
                      <p className="text-[0.75rem] text-slate-800 font-bold">Okupansi: {Math.round((countActiveRooms / totalRooms) * 100)}% ({countActiveRooms}/{totalRooms} Terisi)</p>
                    </div>
                  ) : (
                ...
                <div className="flex border-b-2 border-slate-200 gap-2">
                  <button onClick={() => setActiveTab('penyewa')} className={`py-3 px-5 text-[0.85rem] font-black rounded-t-2xl border-t-2 border-l-2 border-r-2 ${activeTab === 'penyewa' ? 'bg-white border-slate-300 text-emerald-800 -mb-0.5' : 'bg-slate-100 border-transparent text-slate-600'}`}>👥 Daftar Penyewa</button>
                  <button onClick={() => setActiveTab('matrix')} className={`py-3 px-5 text-[0.85rem] font-black rounded-t-2xl border-t-2 border-l-2 border-r-2 ${activeTab === 'matrix' ? 'bg-white border-slate-300 text-slate-900 -mb-0.5' : 'bg-slate-100 border-transparent text-slate-600'}`}>🏠 Matrix Kamar</button>
                </div>
                            ...
                            return (
                              <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="p-3">
                                  ...
                                  <span className="text-[0.65rem] font-mono text-slate-500">Masuk: {t.entry_date}</span>
                                </td>
                                <td className="p-3">
                                  {isHeadPerson ? (
                                    <button onClick={() => handleTogglePaymentStatus(t.id, t.payment_status)} className={`px-2.5 py-1 rounded-xl text-[0.7rem] font-black ${isPaid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{isPaid ? '✅ LUNAS' : '❌ BELUM LUNAS'}</button>
                                ...
                                <td className="p-3">
                                  <span className={'text-[0.65rem] font-black px-2 py-0.5 rounded-full uppercase ' + (t.status === 'VERIFIED' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-900')}>{t.status === 'VERIFIED' ? '✅ TERVERIFIKASI' : '⚠️ MENUNGGU'}</span>
                                </td>
                                <td className="p-3 text-right space-x-1 whitespace-nowrap">
                                  <button onClick={() => handleOpenEditTenant(t)} className="px-2 py-1 bg-amber-400 text-slate-950 text-[0.7rem] font-black rounded-lg">✏️ Edit</button>
                                  <button onClick={() => handleDeleteTenantRow(t.id, t.name)} className="px-2 py-1 bg-slate-200 text-slate-700 text-[0.7rem] font-bold rounded-lg">🗑️</button>
                                </td>
                              </tr>
                            );
                          })}
                ...
                {/* MATRIKS DLL DISINGKAT DEMI FOKUS */}
              </div>
            ) : null}
          </div>
        )}
      ...
      {editingTenant && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm border-2">
            <div className="p-4 bg-emerald-800 text-white flex justify-between"><h3 className="font-black text-[0.85rem]">✏️ Edit Kamar & Nominal</h3><button onClick={() => setEditingTenant(null)}>✕</button></div>
            <form onSubmit={handleSaveTenantSubmit} className="p-5 space-y-3">
              <input type="text" required value={tenantName} onChange={(e) => setTenantName(e.target.value)} className="w-full p-2.5 border-2 rounded-xl" />
              <input type="text" value={tenantRoom} onChange={(e) => setTenantRoom(e.target.value)} className="w-full p-2.5 border-2 rounded-xl" />
              <input type="text" required value={tenantRentPrice} onChange={(e) => setTenantRentPrice(e.target.value)} className="w-full p-2.5 border-2 border-emerald-400 rounded-xl font-mono text-lg" />
              <button type="submit" className="w-full py-2 bg-amber-400 font-black rounded-xl">Simpan Harga</button>
            </form>
          </div>
        </div>
      )}
